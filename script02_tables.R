# Rock Art Analysis Package -- Script 02 Create Tables and Appendix

# Mick Morrison,  22 Feb. 2023

# Environment and requirements
# This script will call clean data from the RDS file produced by 'script01_datacleaning.R' and produce
# a word document with required tables. Development to integrate as a single notebook, avoiding word, is definitely required

# Mick Morrison,  13 July 2024
...
# revisions needed to use csv rather than binary file
data <- readRDS("data/cleandata")

# these can be deleted potentially
...
categories <- fread("motif_category.csv")
...
table01$defineCalculation(
  calculationName = "PercentageAllMotifs",
  type = "calculation",
...
table01$addRowDataGroups(
  "Category",
  totalCaption = "Sub-total",
...
table04$defineCalculation(
  calculationName = "Count",
  summariseExpression = "n()",
  caption = "Count",
  visible = TRUE
)
filterOverrides <- PivotFilterOverrides$new(table04, keepOnlyFiltersFor =
                                              "Count")
table04$defineCalculation(
  calculationName = "TOCTotal",
  filters = filterOverrides,
...
add_border <- function(table) {
  return(border_outer(table, border = fp_border(
    style = "solid",
    width = 1,
    color = "black"
  )))
...
tablesdocx <- body_add_par(
  tablesdocx,
  "Table 4: Motif Contrast state, all sites (1 = heavily weathered, 5 = minimally weathered)"
)
tablesdocx <- body_add_break(tablesdocx)
...
# output the final tables as a single word documenmt.
